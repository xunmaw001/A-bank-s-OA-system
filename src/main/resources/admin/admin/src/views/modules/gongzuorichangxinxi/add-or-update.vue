    ...
    >
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="工作记录" prop="gongzuojilu">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.gongzuojilu" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.gongzuojilu" label="工作记录" prop="gongzuojilu">
                    <span v-html="ruleForm.gongzuojilu"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.baogaoneirong = true;
          ...
          if(o=='gongzuojilu'){
            this.ruleForm.gongzuojilu = obj[o];
	    this.ro.gongzuojilu = true;
            continue;
          }
          if(o=='zhanghao'){
    ...
    info(id) {
      this.$http({
        url: `gongzuorichangxinxi/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	this.ruleForm.gongzuojilu = this.ruleForm.gongzuojilu.replace(reg,'../../../springbootk8uc1/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `gongzuorichangxinxi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.gongzuorichangxinxiCrossAddOrUpdateFlag = false;
                  this.parent.search();
