  ...
  <div>
    <div class="container">
      <div class="login-form" style="backgroundColor:rgba(255, 255, 255, 0.17);borderRadius:10px">
        <h1 class="h1" style="color:rgba(36, 194, 205, 1);fontSize:28px;">某银行OA系统注册</h1>
		<el-form ref="rgsForm" class="rgs-form" :model="rgsForm" label-width="120px">
			...
			<el-form-item label="经理账号" class="input" v-if="tableName=='bumenjingli'">
			...
			<el-form-item label="手机" class="input" v-if="tableName=='bumenjingli'">
			  <el-input v-model="ruleForm.shouji" autocomplete="off" placeholder="手机"  />
			</el-form-item>
			<!-- <div v-if="tableName=='bumenjingli'" class="input-group">
			   <div class="label">身份证</div>
...
export default {
  ...
  methods: {
    ...
    login() {
      ...
      if((!this.ruleForm.jinglizhanghao) && `bumenjingli` == this.tableName){
        this.$message.error(`经理账号不能为空`);
        return
      }
      if((!this.ruleForm.mima) && `bumenjingli` == this.tableName){
        this.$message.error(`密码不能为空`);
        return
      }
      ...
      if(`bumenjingli` == this.tableName && this.ruleForm.shouji&&(!this.$validate.isMobile(this.ruleForm.shouji))){
        this.$message.error(`手机应输入手机格式`);
        return
      }
