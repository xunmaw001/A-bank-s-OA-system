import Vue from 'vue';
//配置路由
import VueRouter from 'vue-router'
Vue.use(VueRouter);
//1.创建组件
import Index from '@/views/index'
import Home from '@/views/home'
import Login from '@/views/login'
import NotFound from '@/views/404'
import UpdatePassword from '@/views/update-password'
import pay from '@/views/pay'
import register from '@/views/register'
import center from '@/views/center'
    import bumenleixing from '@/views/modules/bumenleixing/list'
    import yuangong from '@/views/modules/yuangong/list'
    import baoxiao from '@/views/modules/baoxiao/list'
    import chuqin from '@/views/modules/chuqin/list'
    import gongzuorichangxinxi from '@/views/modules/gongzuorichangxinxi/list'
    import huiyishijian from '@/views/modules/huiyishijian/list'
    import huiyishenqing from '@/views/modules/huiyishenqing/list'
    import bumenjingli from '@/views/modules/bumenjingli/list'
    import qingjia from '@/views/modules/qingjia/list'
    import chucha from '@/views/modules/chucha/list'
    import jilurenwu from '@/views/modules/jilurenwu/list'


//2.配置路由   注意：名字
const routes = [{
    path: '/index',
    name: '首页',
    component: Index,
    children: [{
      // 这里不设置值，是把main作为默认页面
      path: '/',
      name: '首页',
      component: Home,
      meta: {icon:'', title:'center'}
    }, {
      path: '/updatePassword',
      name: '修改密码',
      component: UpdatePassword,
      meta: {icon:'', title:'updatePassword'}
    }, {
      path: '/pay',
      name: '支付',
      component: pay,
      meta: {icon:'', title:'pay'}
    }, {
      path: '/center',
      name: '个人信息',
      component: center,
      meta: {icon:'', title:'center'}
    }
      ,{
	path: '/bumenleixing',
        name: '部门类型',
        component: bumenleixing
      }
      ,{
	path: '/yuangong',
        name: '员工',
        component: yuangong
      }
      ,{
	path: '/baoxiao',
        name: '报销',
        component: baoxiao
      }
      ,{
	path: '/chuqin',
        name: '出勤',
        component: chuqin
      }
      ,{
	path: '/gongzuorichangxinxi',
        name: '工作日常信息',
        component: gongzuorichangxinxi
      }
      ,{
	path: '/huiyishijian',
        name: '会议时间',
        component: huiyishijian
      }
      ,{
	path: '/huiyishenqing',
        name: '会议申请',
        component: huiyishenqing
      }
      ,{
	path: '/bumenjingli',
        name: '部门经理',
        component: bumenjingli
      }
      ,{
	path: '/qingjia',
        name: '请假',
        component: qingjia
      }
      ,{
	path: '/chucha',
        name: '出差',
        component: chucha
      }
      ,{
	path: '/jilurenwu',
        name: '记录任务',
        component: jilurenwu
      }
    ]
  },
  {
    path: '/login',
    name: 'login',
    component: Login,
    meta: {icon:'', title:'login'}
  },
  {
    path: '/register',
    name: 'register',
    component: register,
    meta: {icon:'', title:'register'}
  },
  {
    path: '/',
    name: '首页',
    redirect: '/index'
  }, /*默认跳转路由*/
  {
    path: '*',
    component: NotFound
  }
]
//3.实例化VueRouter  注意：名字
const router = new VueRouter({
  mode: 'hash',
  /*hash模式改为history*/
  routes // （缩写）相当于 routes: routes
})

export default router;
